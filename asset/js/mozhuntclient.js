...
if(typeof $mozhunt === 'undefined')
{
	 ...
	 */
	$mozhunt.comm = {};
	$mozhunt.comm.interval_id;
	$mozhunt.comm.last_hash;
	$mozhunt.comm.cache_bust = 1;
	$mozhunt.comm.attached_callback;
	$mozhunt.comm = {
		/**
		...
		postMessage : function(msg, target_url, target){
			...
			else if(target_url)
			{
				...
				target.location = target_url.replace(/#.*$/, '') + '#' + (new Date) + ($mozhunt.comm.cache_bust++) + '&' + JSON.stringify(msg);
			}
		},
		...
		receiveMessage : function(callback, source_origin){
			...
			if(window['postMessage'])
			{
				...
				if(callback)
				{
					...
					$mozhunt.comm.attached_callback = function(e){
				...
				{
					window[callback ? 'addEventListener' : 'removeEventListener']('message', $mozhunt.comm.attached_callback, !1);
				}
				else
				{
					window[callback ? 'attachEvent' : 'detachEvent']('onmessage', $mozhunt.comm.attached_callback);
				}
			}
			else
			{
				...
				$mozhunt.comm.interval_id && clearInterval(interval_id);
				$mozhunt.comm.interval_id = null;
				
				if(callback)
				{
					$mozhunt.comm.interval_id = setInterval(function(){
						var hash = document.location.hash, re = /^#?\d+&/;
						if(hash !== $mozhunt.comm.last_hash && re.test(hash))
						{
							$mozhunt.comm.last_hash = hash;
							callback({
